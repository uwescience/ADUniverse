...
def make_map():
    ...
    neighborhoods2 = folium.map.FeatureGroup(
        name="neighborhoods2", overlay=True, control=True, show=True,)
    ...
    for i in range(0, len(geo_json_data_test["features"])):
        c = folium.features.GeoJson(geo_json_data_test["features"][i]["geometry"],
                                    ...
                                    style_function=style_function,
                                    highlight_function=highlight_function,)
        folium.Popup(geo_json_data_test["features"][i]["properties"]["name"],
                     geo_json_data_test["features"][i]["properties"]["nhood"]).add_to(c)
