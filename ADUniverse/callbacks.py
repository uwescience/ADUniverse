...
def update_advantage_details(df):
    if (df.iloc[0]["zone_ind"] == 1):
        ...
        if (not pd.isna(df.iloc[0]["sqftfinbasement"])):
            value2 = html.Div(
                ["You have a sizable finished basement that could be converted to an AADU."], id="basement", className='white-box')
            if (df.iloc[0]["daylightbasement"]):
                value2.children.append(
...
def update_dis_details(df):
    if (df.iloc[0]["zone_ind"] == 1):
        ...
        if (df.iloc[0]["yrbuilt"] < 1959):
            value5 = html.Div(
                ["Because your home may be relatively old, if you wish to build an AADU, you may need to find an inspector to ensure no additional changes need to be made to your property."], className='white-box')
